...
void KbdRptParser::OnKeyPressed(uint8_t key) {
  ...
  if (char(key) == 19) {   // Último caracter del código
    ...
    char charAux[200];
    barcode.toCharArray(charAux, 200);  

    LCD.eraseBlock(0,50,maxX,maxY); // Borro la parte donde se va a imprimir el código     
    delay(300);
    LCD.setX((maxX/2)-40); // Muevo el cursor
    LCD.setY((maxY/2 + 20));  
    delay(300);
    LCD.printStr(charAux); // Muestro el código en la pantalla
...
void pintaEstado(char* estado){
    LCD.eraseBlock(0,35,maxX,(maxY)); // Borro la parte del estado anterior     
    delay(300);
    LCD.setX((maxX/2)-43);
    LCD.setY((maxY/2) + 5);  
    delay(300);
...
void compruebaSitio(String barcode) {
  ...
  if (client.connect(server, 80)) {
    ...
    // Make a HTTP request
    client.println("GET /librarinoApp/controladores/ardController.php?id=" + id + "&usuario=" + barcode + " HTTP/1.1");
...
void setup(){
  ...
  String lineaAsiento = "ASIENTO " + id;
  char charAux[200];
  lineaAsiento.toCharArray(charAux, 200);  
  ...
  LCD.setX((maxX/2)-32);
  LCD.setY((maxY/2)-10);  
  LCD.printStr(charAux);
  delay(300);
  LCD.drawLine(0,(maxY/2)-1,maxX,(maxY/2)-1,1);
  delay(300);
  LCD.setX((maxX/2)-43);
  LCD.setY((maxY/2) + 5);
  //////////////////////////////////////////////////////////////////////////////////////
